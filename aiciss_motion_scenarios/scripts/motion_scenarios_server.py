...
'''
 Linear velocities are given in m / s
 Angular velocities are given in rad / s
 
 For safety reasons avoid larger values when working with the real youBot!  
'''
#---- Tunable Parameters -----------------------------------------------------
...
'''
 Defined values produce the following:
 * Square with 1 meter sides (10s, 0.1 m/s)
 * Equilateral triangle with 1 meter sides.
 * Circle with 0.5 meter radius.
'''
#-----------------------------------------------------------------------------
##############################################################################

''' Initial State: wait for a command '''
class InitState(smach.State):
	def __init__(self):
		smach.State.__init__(self,
			outcomes=['init', 'square', 'circle', 'triangle', 'finish'],
...
''' Motion Scenario 1: Follow a squared path '''
class Square(smach.State):
	def __init__(self):
		smach.State.__init__(self,
			outcomes=['square', 'circle', 'triangle', 'finish', 'init'],
...
''' Motion Scenario 2: Follow a circular path '''
...
''' Motion Scenario 3: Follow a triangular path '''
class Triangle(smach.State):
	def __init__(self):
		smach.State.__init__(self,
			outcomes=['square', 'circle', 'triangle', 'finish', 'init'],
...
def do_square():
	...
	publish_velocity(linear_vel, 0, 0, 0, 0, 0, square_time)
	print "-- Drawing line 2 --"	
	publish_velocity(0, linear_vel, 0, 0, 0, 0, square_time)
...
def do_triangle():
	print "-- Drawing line 1 --"
	publish_velocity(linear_vel, 0, 0, 0, 0, 0, triangle_time)
...
def run_state_machine():
	with sm:
		...
		smach.StateMachine.add('Square', Square(),
			transitions={'square' : 'Square', 'circle' : 'Circle', 'triangle' : 'Triangle', 'finish' : 'FINISH', 'init' : 'InitState'},
		...
		smach.StateMachine.add('Triangle', Triangle(),
			transitions={'square' : 'Square', 'circle' : 'Circle', 'triangle' : 'Triangle', 'finish' : 'FINISH', 'init' : 'InitState'},
